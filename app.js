...
window.addEventListener('error', (e) => {
  setStatus('JavaScript error -- see debug.', 'bad');
  showDebug('Error: ' + (e.message || '') + '\n' + (e.error && e.error.stack ? e.error.stack : ''));
});

window.addEventListener('unhandledrejection', (e) => {
  setStatus('Promise error -- see debug.', 'bad');
  showDebug('Unhandled rejection: ' + (e.reason && e.reason.stack ? e.reason.stack : String(e.reason)));
});

setStatus('App loaded. Upload a photo to begin.', 'good');
...
function setControlsEnabled(enabled){
  // Keep buttons clickable; we'll message if user clicks too early.
  setStatus(enabled ? 'Ready. Adjust text and download.' : 'Upload a photo to begin.', 'good');
}
...
async function decodeImageFile(file){
  // Robust decode across browsers, optimized for large images.
  // Returns a CanvasImageSource: ImageBitmap or HTMLImageElement.

  // 1) Try createImageBitmap (fast + reliable on most modern browsers)
  if (window.createImageBitmap){
    try{
      const bmp = await createImageBitmap(file);
      return bmp; // ImageBitmap is drawable via ctx.drawImage
    }catch(err){
      console.warn('createImageBitmap failed', err);
    }
  }

  // 2) Object URL -> HTMLImageElement
  try{
    const url = URL.createObjectURL(file);
    const i = new Image();
    await new Promise((res, rej) => {
      i.onload = () => { URL.revokeObjectURL(url); res(); };
      i.onerror = (e) => { URL.revokeObjectURL(url); rej(e); };
      i.src = url;
    });
    return i;
  }catch(err){
    console.warn('objectURL failed', err);
  }
  ...
  const i = new Image();
  await new Promise((res, rej) => { i.onload=res; i.onerror=rej; i.src=dataUrl; });
  return i;
}
...
async function render(){
  ...
  ctx.save();
  if (supportsFilter){ ctx.filter = `blur(${blurPx}px)`; }
...
els.photo.addEventListener('change', async (e) => {
  ...
  clearDebug();
  setStatus('Loading image...', 'good');

  const started = Date.now();
  const timer = setTimeout(() => {
    setStatus('Still loading... (large photos can take a moment)', 'good');
  }, LOAD_TIMEOUT_MS);
  ...
  try{
    ...
    setStatus(`Rendering... (${ms}ms)`, 'good');

    await render();
    setStatus('Ready. Adjust text and download.', 'good');
  }catch(err){
    clearTimeout(timer);
    console.error(err);
    setStatus('Could not load image -- see debug.', 'bad');
    showDebug('Could not load image.

Details: ' + (err && err.message ? err.message : String(err)) + '

Try: a smaller JPG, a PNG screenshot, or re-export the photo.');
    alert('Could not load that image. See the debug box for details.');
  }
});
...
els.render.addEventListener('click', () => { if(!imgLoaded){ alert('Upload a photo first.'); return; } render(); });
els.download.addEventListener('click', () => { if(!imgLoaded){ alert('Upload a photo first.'); return; } downloadPNG(); });
